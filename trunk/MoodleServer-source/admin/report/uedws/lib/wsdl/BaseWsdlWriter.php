...
class BaseWsdlWriter extends DomDocument
{
    ...
    public function __construct(WsdlDefinition $wsdlDefinition)
    {
        ...
        # enable or disable the wsdl-viewer
        //$this->bWsdlViewer = (boolean) $bWsdlViewer;
     ...
     * @return void
     */
    private function seedXML($bWsdlViewer=true)
    {
        if($bWsdlViewer){
        	$node1 = $this->createProcessingInstruction("xml-stylesheet",'type="text/xsl" href="lib/xsl/wsdl-viewer.xsl"');
    ...
    private function addService()
    {
        ...
        if(!empty($this->description)){
	        $oDocumentation = $this->createElement("documentation" );
	        $oDocumentation->appendChild($this->createTextNode($this->description));
	        $service->appendChild( $oDocumentation );
        }
    ...
    private function addPortTypeOperation($operation)
    {
        ...
        //UED
        $oDocumentation = $this->createElement("documentation" );
        $oDocumentation->appendChild($this->createTextNode($operation->documentation));
        $wsdlOperation->appendChild( $oDocumentation );
